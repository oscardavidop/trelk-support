...
export async function hasPermission(
  ...
  const agent = await Agent.findById(agentId).select(
    "+isActive +permissionsOverride +roleId",
  );
  if (!agent) {
...
export async function invalidatePermissionCache(
  ...
  await Agent.updateOne({ _id: agentId }, { $inc: { permissionVersion: 1 } });
}
...
export async function deleteRole(
  ...
  await Agent.updateMany({ roleId: role._id }, { $unset: { roleId: 1 } });
...
> {
  ...
  > = {
    ...
    "chats.read_all": {
      label: "Ver todos los chats",
      description: "Ver chats de otros agentes",
    },
    "chats.respond": {
      label: "Responder",
      description: "Enviar mensajes en chats",
    },
    ...
    "chats.transfer": {
      label: "Transferir",
      description: "Transferir a otros agentes",
    },
    "chats.takeover": {
      label: "Tomar control",
      description: "Tomar chat de otro agente",
    },
    "chats.delete": {
      label: "Eliminar",
      description: "Eliminar chats individuales",
    },
    "chats.delete_all": {
      label: "Eliminar todos",
      description: "⚠️ Eliminar todos los chats",
    },
    "chats.monitor": {
      label: "Monitorear",
    ...
    "chats.export": {
      label: "Exportar",
      description: "Exportar transcripciones",
    },

    // Uploads
    "uploads.upload": {
      label: "Subir archivos",
      description: "Subir archivos en chats",
    },
    "uploads.delete": {
      label: "Eliminar archivos",
      description: "Eliminar archivos subidos",
    },
    ...
    "contacts.delete": {
      ...
      description: "⚠️ Eliminar contactos",
    },
    "contacts.export": {
    ...
    "contacts.block": {
      ...
      description: "Bloquear/desbloquear",
    },
    "contacts.merge": {
      label: "Fusionar contactos",
      description: "Fusionar duplicados",
    },

    // Agents
    "agents.read": {
      label: "Ver agentes",
      description: "Ver lista de agentes",
    },
    "agents.write": {
      label: "Gestionar agentes",
      description: "Crear/editar agentes",
    },
    "agents.delete": {
      label: "Eliminar agentes",
      description: "⚠️ Eliminar agentes",
    },
    "agents.permissions": {
    ...
    "agents.status": {
      ...
      description: "Cambiar estado de agentes",
    },
    "agents.teams": {
      label: "Gestionar equipos",
      description: "Asignar a equipos",
    },
 ...
 */
export async function getAgentPermissionsSummary(
  agentId: string,
): Promise<AgentWithPermissions | null> {
  const agent = await Agent.findById(agentId);
  if (!agent) return null;
