        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70 backdrop-blur-sm animate-in fade-in">
          <div className="bg-zinc-900 border border-zinc-800 rounded-2xl w-full max-w-md shadow-2xl p-6">
            ...
            <div className="bg-zinc-950/50 rounded-xl p-4 mb-6 border border-zinc-800/50">
              <p className="text-xs text-zinc-500 uppercase font-bold mb-2 tracking-wider">Vista Previa</p>
      ...
      <div className="flex justify-between items-start mb-4">
        <div className="space-y-1 pr-4 min-w-0">
          <div className="flex items-center gap-3">
            ...
            <span className={`inline-flex items-center gap-1.5 px-2 py-0.5 rounded-full text-[10px] font-bold border uppercase tracking-wider ${status.bg} ${status.border} ${status.color}`}>
              {status.icon}
              {status.label}
            </span>
          </div>
        ...
        <div className="flex items-start justify-between px-8 py-6 border-b border-zinc-800 bg-zinc-900">
          <div>
            <div className="flex items-center gap-3 mb-1">
              ...
              <span className={`px-2 py-0.5 rounded-full text-[10px] uppercase font-bold border ${status.bg} ${status.color} ${status.border}`}>{status.label}</span>
            </div>
            ...
            <div className="lg:col-span-2 bg-zinc-950/30 border border-zinc-800 rounded-xl overflow-hidden flex flex-col h-[500px]">
              ...
              <div className="flex-1 overflow-auto custom-scrollbar relative">
                ...
                <table className="w-full text-left border-collapse">
                  <thead className="bg-zinc-900 text-xs font-bold text-zinc-500 uppercase tracking-wider sticky top-0 z-0">
                    <tr>
                      ...
                      return (
                        <tr key={r._id} className="hover:bg-white/[0.02]">
                          ...
                          <td className="px-4 py-2.5">
                            <span className={`inline-flex items-center gap-1 px-2 py-0.5 rounded text-[10px] font-bold uppercase tracking-wider ${statusConfig.bg} ${statusConfig.color}`}>
...
function StatCard({ label, value, icon: Icon, color, bg }: any) {
  return (
    <div className={`flex flex-col items-center justify-center p-4 rounded-xl border border-zinc-800/50 ${bg} bg-opacity-5`}>
      ...
      <span className="text-[10px] uppercase font-bold text-zinc-500 tracking-wider mt-1">{label}</span>
    </div>
  );
}
