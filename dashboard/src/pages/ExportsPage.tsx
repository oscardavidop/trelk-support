        ...
        <div className="px-8 py-6 pb-2">
          <div className="flex items-center justify-between mb-6">
            <div className="flex items-center gap-4">
              ...
              <div>
                <h1 className="text-2xl font-bold text-white tracking-tight">Centro de Exportación</h1>
        ...
        <div className="flex-1 min-w-0 grid grid-cols-1 md:grid-cols-4 gap-4 items-center">
          <div className="col-span-1">
            <div className="flex items-center gap-2 mb-1">
              <span className={`text-xs font-bold uppercase tracking-wider px-1.5 rounded border ${formatConfig.bg} ${formatConfig.color} ${formatConfig.border}`}>
                {job.format}
              </span>
              <span className="text-zinc-500 text-xs">•</span>
              <span className="text-zinc-400 text-xs font-mono">{new Date(job.createdAt).toLocaleDateString()}</span>
            </div>
            <div className="text-sm font-medium text-white truncate">Exportación de datos</div>
          </div>
          ...
          <div className="col-span-1 flex items-center justify-end gap-2">
            {job.status === 'completed' && job.downloadUrl && (
              <a
                href={job.downloadUrl}
                download
                className="flex items-center gap-2 px-3 py-1.5 bg-emerald-600 hover:bg-emerald-500 text-white rounded-lg text-xs font-medium transition-colors shadow-lg shadow-emerald-900/20"
              >
                <Download className="w-3.5 h-3.5" /> Descargar
              </a>
            )}
            <button onClick={() => setExpanded(!expanded)} className="p-1.5 text-zinc-500 hover:text-white hover:bg-zinc-800 rounded-lg transition-colors">
      ...
      {expanded && (
        <div className="px-4 pb-4 pt-0 border-t border-zinc-800/50 bg-zinc-950/30">
          <div className="grid grid-cols-2 gap-8 py-4">
            <div>
              <h4 className="text-xs font-bold text-zinc-500 uppercase tracking-widest mb-2">Filtros Aplicados</h4>
              <div className="flex flex-wrap gap-2">
            ...
            <div>
              <h4 className="text-xs font-bold text-zinc-500 uppercase tracking-widest mb-2">Contenido</h4>
      ...
      <div className="w-full max-w-2xl bg-zinc-900 border border-zinc-800 rounded-2xl shadow-2xl overflow-hidden flex flex-col max-h-[90vh]">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-zinc-800 bg-zinc-900">
          <div className="flex items-center gap-3">
            ...
            <h2 className="text-lg font-bold text-white">Nueva Exportación</h2>
          </div>
        ...
        <div className="p-6 overflow-y-auto custom-scrollbar space-y-8">
          ...
          {/* 1. Format Selection */}
          <div className="space-y-3">
            <label className="text-xs font-bold text-zinc-500 uppercase tracking-widest">Formato de Archivo</label>
          ...
          {/* 2. Filters */}
          <div className="space-y-3">
            <label className="text-xs font-bold text-zinc-500 uppercase tracking-widest">Filtros de Tiempo</label>
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="block text-xs text-zinc-400 mb-1">Desde</label>
                <input type="date" value={dateFrom} onChange={(e) => setDateFrom(e.target.value)} className="w-full px-3 py-2 bg-zinc-950 border border-zinc-800 rounded-lg text-white focus:border-emerald-500 focus:outline-none" />
              </div>
              <div>
                <label className="block text-xs text-zinc-400 mb-1">Hasta</label>
                <input type="date" value={dateTo} onChange={(e) => setDateTo(e.target.value)} className="w-full px-3 py-2 bg-zinc-950 border border-zinc-800 rounded-lg text-white focus:border-emerald-500 focus:outline-none" />
              </div>
            </div>
          </div>
          ...
          {/* 3. Agents */}
          <div className="space-y-3">
            <label className="text-xs font-bold text-zinc-500 uppercase tracking-widest">Filtrar por Agente</label>
            <select
              multiple
              value={selectedAgents}
              onChange={(e) => setSelectedAgents(Array.from(e.target.selectedOptions, o => o.value))}
              className="w-full px-3 py-2 bg-zinc-950 border border-zinc-800 rounded-lg text-white focus:border-emerald-500 focus:outline-none h-32"
            >
          ...
          {/* 4. Content Toggles */}
          <div className="space-y-3">
            <label className="text-xs font-bold text-zinc-500 uppercase tracking-widest">Incluir Datos</label>
        ...
        <div className="flex justify-end gap-3 px-6 py-4 bg-zinc-900/50 border-t border-zinc-800">
          ...
          <button
            onClick={onSubmit}
            disabled={creating}
            className="flex items-center gap-2 px-6 py-2.5 bg-emerald-600 hover:bg-emerald-500 text-white font-medium rounded-xl shadow-lg shadow-emerald-900/20 transition-all disabled:opacity-50"
          >
...
const FormatButton = ({ active, onClick, icon: Icon, label, desc, color, border, bg }: any) => (
  ...
  >
    ...
    <span className={`font-bold text-sm ${active ? 'text-white' : 'text-zinc-400'}`}>{label}</span>
